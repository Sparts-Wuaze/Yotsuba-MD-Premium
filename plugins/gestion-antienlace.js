...
const handler = async (m, { conn, usedPrefix, command, args, isAdmin, isOwner }) => {
  ...
  if (['antienlace', 'antilink'].includes(command)) {
    if (!(isAdmin || isOwner)) {
      return conn.reply(m.chat, '🤨 Only administrators can activate or deactivate the anti-link.', m, rcanal)
    }
    if (args[0] === 'on' || args[0] === 'enable') {
      if (isAntiLink) return conn.reply(m.chat, `💜 The function *${command}* was already *activated*.`, m, rcanal)
      isAntiLink = true
    } else if (args[0] === 'off' || args[0] === 'disable') {
      if (!isAntiLink) return conn.reply(m.chat, `👑 The function *${command}* was already *disabled*.`, m, rcanal)
      isAntiLink = false
    } else {
      return conn.reply(
        m.chat,
    ...
    return conn.reply(m.chat, `👑 The function *${command}* was *${isAntiLink ? 'activated' : 'deactivated'}* for this group.`, m, rcanal)
  }
}
...
export async function detectLink(conn, m) {
  ...
  if (linkRegex.test(m.text)) {
    // Responder primero
    await conn.sendMessage(m.chat, { text: '🤨 Don't send the link if you don't want me to delete you.' }, { quoted: m })
